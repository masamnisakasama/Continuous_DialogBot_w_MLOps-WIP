...
一旦アプリにした後は、プレゼン音声と成約可否のラベルつき擬似データを作ってMLOpsする予定です。
...
<br><br>＜注意＞<br>
- 音声解析（STT（Speech To Text））が遅いです。Whisper API（音声をテキストにするOpen AIのAPI）がデフォルトで遅いのが原因なので、Whisper APIを使う限り技術でどうにかできる類の問題ではなさそうです。大体5分の音声で10分前後の処理時間をご用意ください。<br>
...
## 主な機能
🎙️ 音声からテキストへ (STT) : Whisper API を利用した文字起こし
🧠 スコアリング : 構造性・一貫性・冗長性（密度）・CTA などを LLM で評価
⏱️ 音声指標 : フィラー数、発話速度、ポーズ比率、抑揚の簡易指標
🖥️ Web UI : React 製のシンプルな解析フロントエンド
☁️ 保存 : 将来の再学習に向けた S3 等への保存に対応予定（WIP）
...
S3については、一時期分析結果を表示していたものの、セキュリティーの観点からフロントエンド側から見れないようにしました。将来のMLOpsへの拡張のために集計自体はされているものの、デフォルトで暗号化が効いてますし、中身ももちろん見ませんのでご安心ください。<br>
...
- ローカル環境で大体のメモリ使用量を測り、用いるバックエンドのデプロイサービスを見極める必要がある。
