...
<br><br>＜注意＞<br>
- **音声解析（STT（Speech To Text））が遅い**です。Whisper API（音声をテキストにするOpen AIのAPI）がデフォルトで遅いのが原因なので、Whisper APIを使う限り技術でどうにかできる類の問題ではなさそうです。大体5分の音声で10分前後の処理時間をご用意ください。<br>
- **PC(MacOS/Windows)からご利用**ください。ファイルサイズはWhisper APIの処理の関係上24 MBまでです。
